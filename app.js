...
function openPlayer(song,mode='play'){
  ...
  if(audio){$('player-content').innerHTML=`<img class="audio-art" src="${esc(song.image)}" alt="${esc(song.title)}"><audio controls controlslist="nodownload" preload="metadata" src="${esc(audio)}"></audio><p class="player-footer" id="audio-status" role="status"></p>`;const player=$('player-content').querySelector('audio');player.addEventListener('error',()=>{$('audio-status').textContent='La piste ne peut pas être lue. Vérifiez son lien ou réessayez plus tard.';},{signal:playerAbort.signal});if(timing)loadTiming(player,timing,playerAbort.signal);else if(v.lyrics)$('player-extra').innerHTML=`<div class="timed-lyrics lyrics">${esc(v.lyrics)}</div>`;
