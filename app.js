...
document.addEventListener('click',event=>{
  document.querySelectorAll('.song-info:not([hidden])').forEach(panel=>{const owner=document.querySelector(`[aria-controls="${panel.id}"]`);if(!panel.contains(event.target)&&event.target!==owner){panel.hidden=true;owner?.setAttribute('aria-expanded','false');}});
