...
function render(){
  ...
  const filtered=songs.filter(s=>(state.theme==='Toutes'||s.tags.includes(state.theme))&&(state.language==='all'?Object.values(s.versions).some(hasReleaseDate):hasReleaseDate(s.versions[state.language]))).sort((a,b)=>versionOf(b).version.releaseDate.localeCompare(versionOf(a).version.releaseDate));
